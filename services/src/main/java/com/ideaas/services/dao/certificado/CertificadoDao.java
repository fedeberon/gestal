...
public interface CertificadoDao extends JpaRepository<Certificado, Long> {
    ...
    @Query( value = "SELECT SUM(CER_AUSENTISMO) FROM CERTIFICADOS WHERE MONTH(CER_FECHA_INICIO) AND YEAR (CER_FECHA_INICIO) = YEAR(CURDATE());", nativeQuery = true)
    ...
    @Query( value = "SELECT COL_NAME,SUM(CERTIFICADOS.CER_AUSENTISMO) AS AUSENCIAS FROM CERTIFICADOS JOIN COLABORADORES ON CERTIFICADOS.CER_COL_ID = COLABORADORES.COL_ID JOIN SUCURSALES ON COLABORADORES.COL_SUC_ID = SUCURSALES.SUC_ID GROUP BY COL_ID;", nativeQuery = true)
