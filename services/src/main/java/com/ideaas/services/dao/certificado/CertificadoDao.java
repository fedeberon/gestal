...
public interface CertificadoDao extends JpaRepository<Certificado, Long> {
    ...
    List<Certificado> findCertificadoByFechaInicioBetween(String fechaInicio, String fechaFin);
    ...
    @Query( value = "SELECT SUM(CER_AUSENTISMO) FROM CERTIFICADOS WHERE MONTH(CER_FECHA_INICIO) = MONTH(CURRENT_DATE()) AND YEAR(CER_FECHA_INICIO)=YEAR(CURDATE()); ", nativeQuery = true)
